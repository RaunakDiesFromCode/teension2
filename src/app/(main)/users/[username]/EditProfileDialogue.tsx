...
function AvatarInput({ src, onImageCropped }: ImageInputProps) {
...
function CoverPhotoInput({ src, onImageCropped }: ImageInputProps) {
  const [imageToCrop, setImageToCrop] = useState<File>();
  const fileInputRef = useRef<HTMLInputElement>(null);
  ...
  function onImageSelected(image: File | undefined) {
    ...
    Resizer.imageFileResizer(
      image,
      1366, // Set to the desired width for cover photos
      768, // Set to the desired height for cover photos
      "WEBP",
      100,
      0,
      (uri) => setImageToCrop(uri as File),
  ...
  return (
    <>
      ...
      {imageToCrop && (
        <CropImageDialog
          src={URL.createObjectURL(imageToCrop)}
          cropAspectRatio={16 / 9} // Use 16:9 for cover photos
          onCropped={onImageCropped}
          onClose={() => {
            setImageToCrop(undefined);
            if (fileInputRef.current) {
